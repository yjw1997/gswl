<template>
  <global-layout @dynamicRouterShow="dynamicRouterShow">
    ...
    <div style="display:flex;background:#fff">
      ...
      <a-icon v-if="multipage" id="close-menu" type="close-circle" class="close-btn" theme="twoTone" @mousedown="e => onContextmenu(e,true)" />
    </div>
...
export default {
  ...
  created() {
    ...
    const storeKey = 'route:title:' + this.$route.fullPath
    const routeTitle = this.$ls.get(storeKey)
    if (routeTitle) {
  ...
  methods: {
    ...
    onMenuSelect(key, target) {
      const pageKey = this.getPageKey(target) || this.$route.path
      switch (key) {
        case '1':
