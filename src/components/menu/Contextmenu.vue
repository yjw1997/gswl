...
export default {
  ...
  created() {
    window.addEventListener('mousedown', (e, isCloseBtn) => {
      if (e.target.tagName === 'path') { // 按钮svg
        this.setPosition(e)
      } else {
        this.closeMenu(e)
      }
  ...
  methods: {
    ...
    setPosition(e) {
      this.left = e.clientX
      if (document.body.clientWidth - e.clientX < 126) {
        this.left = e.clientX - 126 // 126菜单选项宽度
      }
