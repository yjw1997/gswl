...
import { deleteAction, getAction, downFile, postHeaderAction, editStatus } from '@/api/manage'
import Vue from 'vue'
...
export const JeecgListMixin = {
  ...
  methods: {
    ...
    handleDeleteStatus: function(record) {
      ...
      console.log('anniu', record.delFlag === 0)
    ...
    //  外部订单单条接收拒绝
    hadnleIsAcceptOrder: function(id, flag) {
      console.log('外部订单ID', id)
      if (!this.url.delete) {
        this.$message.error('请设置url.delete属性!')
        return
      }
      ...
      this.$confirm({
        ...
        onOk: function() {
          that.loading = true
          postHeaderAction(that.url.delete, {
            orderIds: id,
            flag: flag
          })
            .then(res => {
    ...
    //  外部订单多条接收拒绝
    hadnleIsAcceptOrders: function(flag) {
      if (!this.url.deleteBatch) {
        this.$message.error('请设置url.deleteBatch属性!')
        return
      }
      if (this.selectedRowKeys.length <= 0) {
        ...
        this.$confirm({
          ...
          onOk: function() {
            that.loading = true
            postHeaderAction(that.url.deleteBatch, {
              orderIds: ids,
              flag: flag
            })
              .then(res => {
